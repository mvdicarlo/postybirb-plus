 ...
 * @class Http
 */
export class HttpExperimental {
  private static logger: Logger = new Logger(HttpExperimental.name);
  ...
  private static createClientRequest(
    ...
    if (options.queryParameters) {
      ...
      url.search = new URLSearchParams(encodeQueryString(options.queryParameters)).toString();
  ...
  private static createPostBody(options: PostOptions | BinaryPostOptions): CreateBodyData {
  ...
  private static handleError(req: ClientRequest, reject: (reason?: Error) => void): void {
  ...
  static get<T>(
    ...
    return new Promise((resolve, reject) => {
      const req = HttpExperimental.createClientRequest(options, {
        ...(crOptions ?? {}),
        url,
      });
      ...
      if (typeof body === 'string' && HttpExperimental.isOnCloudFlareChallengePage(body)) {
        ...
        return HttpExperimental.performBrowserWindowGetRequest<T>(url, options, crOptions);
      }
  ...
  private static async performBrowserWindowPostRequest<T>(
    ...
    const { contentType, buffer } = HttpExperimental.createPostBody(options);
    const headers = Object.entries({ ...(options.headers ?? {}), 'Content-Type': contentType })
  ...
  private static async awaitCloudFlareChallengePage(window: BrowserWindow): Promise<void> {
